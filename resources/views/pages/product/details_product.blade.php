...
@foreach ( $details_product as $rs )
    <div class="product-details"><!--product-details-->
        <div class="col-sm-5">
            ...
            <img src="{{ asset('public/uploads/product/'.$rs->product_image) }}" alt="" />
            </div>
        ...
        <div class="col-sm-7">
            <div class="product-information"><!--/product-information-->
                ...
                <h2>{{ $rs->product_name }}</h2>
                <p>Mã sản phẩm : {{ $rs->product_id }}</p>
                <img src="{{ asset('frontend/images/product-details/rating.png') }}" alt="" />
                <form action="{{ URL::to('/save-cart') }}" method="post">
                    @csrf
                    <span>
                        <span>{{ number_format($rs->product_price).'VNĐ' }}</span>
                        <label>Số lượng:</label>
                        <input type="number" name="qty" min ="1" value="3" />
                        <input name="productid_hidden" type="hidden" value="{{ $rs->product_id }}" />
                        <button style="margin:15px;" type="submit" class="btn btn-fefault cart">
                ...
                <p><b>Thương hiệu:</b> {{ $rs->brand_name }}</p>
                        ...
                        <div class="product-image-wrapper">
                            <div class="single-products">
                                <div class="productinfo text-center">
                                    ...
                                    <a href="{{ URL::to('save-cart') }}"><button type="button" class="btn btn-default add-to-cart"><i class="fa fa-shopping-cart"></i>Thêm vào giỏ</button></a>
                                </div>
                            </div>
                        </div>
                    </div>
